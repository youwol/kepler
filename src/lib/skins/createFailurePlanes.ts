...
export function createFailurePlanes({
    ...
    const mng = new Manager(dataframe, {
        decomposers: [new ComponentDecomposer(), new EigenValuesDecomposer()],
        dimension: 3,
    })
    ...
    eigv.forEach((vectors, i) => {
        ...
        if (parameters.type === FractureType.FAULT) {
            ...
            parameters.type === FractureType.JOINT ||
            parameters.type === FractureType.DYKE
        ) {
            rot.setFromUnitVectors(
                new Vector3(0, 0, 1),
                new Vector3(vectors[0], vectors[1], vectors[2]),
            )
        } else if (parameters.type === FractureType.STYLOLITE) {
            rot.setFromUnitVectors(
                new Vector3(0, 0, 1),
                new Vector3(vectors[3], vectors[4], vectors[5]),
            )
        }
