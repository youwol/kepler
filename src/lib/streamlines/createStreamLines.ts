...
export class StreamLinesParameters {
    ...
    constructor(
        {onStreamlineAddedCB, seed, dTest=0.25, dSep=0.5, timeStep=0.01, forwardOnly=false}:
        {onStreamlineAddedCB: Function, seed?: Point2D, dTest?: number, dSep?: number, timeStep?: number, forwardOnly?:boolean})
    {
        this.seed = seed
        this.dTest = dTest
        this.dSep = dSep
 ...
 */
export function createStreamLines(positions: Serie, indices: Serie, field: Serie, params: StreamLinesParameters) {
    if (field.itemSize !== 3) {
    ...
    if (params.onStreamlineAddedCB === undefined) throw new Error('onStreamlineAddedCB must be set for streamlines')
    ...
    return computeStreamlines({
        vectorField(p: Point2D) {
            ...
            return triangleLerp2D(
                p, 
                position2D(i1, positions), position2D(i2, positions), position2D(i3, positions),
                field.itemAt(i1) as number[], field.itemAt(i2) as number[], field.itemAt(i3) as number[]
            )
        },
        boundingBox : bboxs,
        seed        : params.seed,
...
// bbox of a triangle
const bbox = (positions: Serie, i1: number, i2: number, i3: number): Triangle => {
...
const dot2  = (a: number[], b: number[]): number  => a.reduce( (acc, cur, i) => acc+cur*b[i])

const from2 = (a:Point2D, b: Point2D): number[] => [b.x-a.x, b.y-a.y]

function barycentric2(p: Point2D, a: Point2D, b: Point2D, c: Point2D): number[] {
    const v0 = from2(a, b)
    const v1 = from2(a, c)
...
function triangleLerp2D(p: Point2D, p1: Point2D, p2: Point2D, p3: Point2D, q1: number[], q2: number[], q3: number[]) {
